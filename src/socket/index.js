...
module.exports.callSocket =  function(server){
    ...
    // START IO
        ...
        socket.on("userlogin",async function (user_id,peer_id){
            ...
            // console.log(Globalclients[user_id], Globalclients)
            Clients.overView()
        ...
        socket.on("request_join_room", async function(request_user_id, room_id){
            ...
            console.log(existsRequest.data.length)
            console.log(existsRequest.data)
        });
            ...
            if(type == "accept"){
                ...
                let dataAfterAccept = await roomsModel.get({ 
                    ...
                    join: [
                        {
                            type : "LEFT JOIN",
                            table: `(SELECT * FROM dtb_rooms_users WHERE user_id = ${request_user_id} GROUP BY room_id ) AS temp_dru `,
                            on   : "ON",
